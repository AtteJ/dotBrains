...
struct DotView: View {
    @ObservedObject var dot: Species
    let dotSize: CGFloat
    
    init(_ dot: Species, _ size: CGFloat) {
        self.dot = dot;
        self.dotSize = size
    }
    ...
    func dotColor(dot: Species) -> Color {
        if dot.success {
            return .green
        }
...
struct ContentView: View {
    ...
    var body: some View {
        return ZStack {
            ...
            ForEach (population.dots) { (dot: Species) in
                DotView(dot, 10)
            }
